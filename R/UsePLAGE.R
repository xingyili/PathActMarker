...
#' data(KICHcli)
#' PLAGEResults = UsePLAGE(KICHfilter, HallmarkgenesetsID)
...
.plage <- function(exp, pathwaylist,
  ...
  if (rnaseq)
    stop("rnaseq=TRUE does not work with method='plagezhao'.")

  if(verbose)
    cat("Estimating plagezhao scores for", length(pathwaylist),"gene sets.\n")
