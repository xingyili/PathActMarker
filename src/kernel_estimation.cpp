...
void
matrix_density_R(Rcpp::NumericVector t_X, Rcpp::NumericVector t_Y, Rcpp::NumericVector t_R,
                 Rcpp::IntegerVector t_n_density_samples, Rcpp::IntegerVector t_n_test_samples,
                 Rcpp::IntegerVector t_n_genes, Rcpp::IntegerVector t_rnaseq);
  ...
  if (!rnaseq && is_precomputed == 0) {
	...
	for(int j = 0; j < size_test_n; ++j){
		...
		for(int i = 0; i < size_density_n; ++i){
		  left_tail += rnaseq ? R::ppois(y[j], x[i]+bw, TRUE, FALSE) : precomputedCdf(y[j]-x[i], bw);
		}
...
// [[Rcpp::export]]
void matrix_density_R(Rcpp::NumericVector t_X, Rcpp::NumericVector t_Y, Rcpp::NumericVector t_R,
                      Rcpp::IntegerVector t_n_density_samples, Rcpp::IntegerVector t_n_test_samples,
                      Rcpp::IntegerVector t_n_genes, Rcpp::IntegerVector t_rnaseq){
  double* X = t_X.begin();
  double* Y = t_Y.begin();
  double* R = t_R.begin();
  int* n_density_samples = t_n_density_samples.begin();
  int* n_test_samples = t_n_test_samples.begin();
  int* n_genes = t_n_genes.begin();
  int* rnaseq = t_rnaseq.begin();
	matrix_d(X, Y, R, *n_density_samples,*n_test_samples, *n_genes, *rnaseq);
}
...
void initCdfs(void){
    ...
    precomputed_cdf[i] = R::pnorm5(MAX_PRECOMPUTE * ((double) i) / divisor, 0.0, 1.0, TRUE, FALSE);
